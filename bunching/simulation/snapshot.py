...
class Snapshot:
    def __init__(self, episode, ct, curr_bus_id, curr_stop_id, curr_trip_no, buses, stops, config) -> None:
        '''
        ...
        self.curr_trip_no = curr_trip_no
        # get corridor length for loop corridor
        last_link_id = max(config.link_info.keys())
        self.__corri_lengt = config.link_info[last_link_id]['end_x']
    ...
    def get_spacing_state(self, buses, curr_bus_id):
        ...
        local_state = [for_ward_x/self.__corri_lengt,
                       back_ward_x/self.__corri_lengt]
        globa_state = [x/self.__corri_lengt for x in globa_xs]
    ...
    def cal_relative_global_state(self, xs, x):
        ...
        '''
        if self.bus_num % 2 == 0:
            # TODO need to be done in the future
            # even bus num
            pass
        else:
            curr_idx = xs.index(x)
            ...
            for dt in range(1, for_bus_num+1, 1):
                ...
                if idx <= (self.bus_num-1):
                    ...
                    for_spacs.append(
                        xs[idx % self.bus_num] + self.__corri_lengt - x)
