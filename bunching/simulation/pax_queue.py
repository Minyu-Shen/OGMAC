...
# class Pax:
#     def ___init__(self, aligh_probs) -> None:
#         dest = np.random.choice(len(aligh_probs), 1, p=aligh_probs)[0]
#         self.__dest = dest
...
class Pax_Queue:
    ...
    def reset(self):
        ...
        self.__dest_queue = {dest: 1.0 for dest, _ in self.__dest_rate.items()}

    def arrive(self, ct, dt):
    ...
    def board(self, bus, dt):
        ...
        for dest in dests:
            ...
            # minim = min(self.__dest_queue[dest], quota)
            # self.__dest_queue -= minim
            # return dest, minim
