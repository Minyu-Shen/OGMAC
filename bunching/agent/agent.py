...
class Agent:
    def __init__(self, config, agent_config, is_eval) -> None:
        ...
        if agent_config['agent_name'] in ['NONLINEAR', 'DO_NOTHING', 'IMAGINATION']:
        ...
        # self._bus_stop_trip_occup_rate_dict = {}

    def reset(self, episode, is_record_wandb=True, G=None):

        print('G is:', G)
        sum_rewar = sum(self._track_rewas) / len(self._track_rewas)
        equal_rewar = sum(self._track_equal_rewas) / \
            len(self._track_equal_rewas)
        inten_rewar = sum(self._track_inten_rewas) / \
            len(self._track_inten_rewas)
        hold_time = sum(self._track_hold_times) / len(self._track_hold_times)
        ...
        if is_record_wandb:
            wandb.log({'sum': sum_rewar, 'equalized reward': equal_rewar, 'intensity reward': inten_rewar, 'headway variation': headw_varia,
                      'hold time': hold_time, 'waiting time': avg_wait_time, 'riding time': avg_ride_time, 'mc_return': G})
        ...
        # record the onboard occupancy rate of each bus when departing from a stop
        # (bus_id, stop_id, trip_no) -> occup_rate
        # self._bus_stop_trip_occup_rate_dict = {}
    ...
    # def track_onboard_occupancy_rate(self, snapshot):
    #     self._bus_stop_trip_occup_rate_dict[(
    #         snapshot.curr_bus_id, snapshot.curr_stop_id, snapshot.curr_trip_no)] = snapshot.curr_occup_rate
